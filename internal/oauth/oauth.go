...
// LoadAndValidateToken loads a token and refreshes it if expired
func LoadAndValidateToken(store *nokeyKeyring.Store, provider Provider) (*Token, error) {
	providerName := provider.GetProviderName()

	// Load token
	token, err := LoadToken(store, providerName)
	if err != nil {
		return nil, fmt.Errorf("no OAuth token found for %s (run: nokey auth oauth setup --provider %s)",
	...
	if token.IsExpired() {
		if token.RefreshToken == "" {
			return nil, fmt.Errorf("OAuth token expired and no refresh token available (run: nokey auth oauth setup --provider %s)",
				providerName)
		}
		...
		if err := SaveToken(store, providerName, newToken); err != nil {
	...
	if err := provider.ValidateToken(ctx, token); err != nil {
		return nil, fmt.Errorf("OAuth token validation failed: %w (run: nokey auth oauth setup --provider %s)",
			err, providerName)
	}
